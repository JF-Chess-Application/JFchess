class Piece < ApplicationRecord
  ...
  def check_diagonal_obstructions(x, y)
    ...
    # Loop through every square in range
    y_range(y).each_with_index do |pointer_y, i|
      x_range(x).each_with_index do |pointer_x, j|

        # if desired move is diagonal up and to the right or down and to the left
        if (x > position_x && y > position_y) || (x < position_x && y < position_y)
          if i == j
            return true if game.piece_in_space?(pointer_x, pointer_y)
          end

        # if desired move is diagonal up and to the left or down and to the right
        elsif (x < position_x && y > position_y) || (x > position_x && y < position_y)
          if x_range(x).count - i - 1 == j
            return true if game.piece_in_space?(pointer_x, pointer_y)
          end
        end
      end
